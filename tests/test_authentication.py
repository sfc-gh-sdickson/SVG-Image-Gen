...
class TestAuthentication:
    ...
    @pytest.mark.skipif(not SNOWFLAKE_AVAILABLE, reason="Snowflake packages not available")
    def test_get_active_session_fails_outside_snowflake(self):
    ...
    def test_snowflake_environment_session(self, mock_get_active_session):
        """Test successful session creation in Snowflake environment."""
        # Mock a successful active session
        mock_session = Mock()
    ...
    def test_local_environment_session_with_env_vars(self, mock_session_builder, mock_get_active_session):
    ...
    @patch('snowflake.snowpark.context.get_active_session')
    def test_local_environment_missing_env_vars(self, mock_get_active_session):
        ...
        with patch.dict(os.environ, {var: '' for var in snowflake_vars}, clear=True):
            ...
            with pytest.raises(Exception):
                ...
                pass  # This would be handled in the actual app with st.stop()
...
class TestSessionConfiguration:
    ...
    @patch('snowflake.snowpark.Session.builder')
    def test_session_configuration_with_all_params(self, mock_session_builder):
    ...
    @patch('snowflake.snowpark.Session.builder')
    def test_session_configuration_minimal_params(self, mock_session_builder):
